...
import OrderMarkovModel as omk
import os
import sys
import time
...
db_name = "experiment_10k.db"
dbops = db_ops.dbops(db_name)


myModel  = omk.Model()

myModel.trainModelfromDB(dbops)
myModel.printModel()
...
for i in range(1, test_set_size + 1):
	...
	prediction[i] = myModel.predict(dom_name)
	print "predicting " + str(i) + " - " + dom_name + " - " + str(prediction[i])
